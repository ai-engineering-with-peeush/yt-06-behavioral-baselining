...
STARTER STATE (A2 pre-recording): extract_row() builds a 5-feature vector.
novel_device_and_unusual_hour is imported below but never called — a
silent train/eval parity bug, even though src/detector.py already expects
all 6 features. Alerting is IsolationForest-score-only; no OR-rule exists
yet. Diff this file against ../evals/run_eval.py to see the fix.
"""
...
def extract_row(
    ...
    """Build the 5-feature vector for one login event.

    BUG: novel_device_and_unusual_hour is imported above but never called
    here, so it never reaches the model — even though src/detector.py
    already builds a 6-feature vector that includes it.
    ...
    return [vel, nov, hdev, inter, burst]
...
def train(
    ...
    all_events: List[LoginEvent],
    histories: Dict[str, UserHistory],
) -> Tuple[IsolationForest, float, np.ndarray, List[str], np.ndarray, np.ndarray]:
    ...
    return detector, threshold, X_all_std, eids, mu, sigma
...
def print_report(
    ...
    print("\n" + "=" * 65)
    print("  Alerts fire on IsolationForest score < threshold only.")
    print("  novel_device_and_unusual_hour is imported but unused.")
    print("=" * 65 + "\n")
...
if __name__ == "__main__":
    ...
    detector, threshold, X_all_std, scored_eids, mu, sigma = train(
    ...
    if_alerts = {
        eid: bool(raw_scores[i] < threshold) for i, eid in enumerate(scored_eids)
    }
